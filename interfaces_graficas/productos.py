...
def productos():
    ...
    frame_superior = tk.Frame(ventana_productos, bg=color_principal)
    frame_superior.grid(row=0, column=0, sticky="ew", padx=20, pady=(20, 10))


    # BUSCADOR A LA IZQUIERDA
    label_buscar = tk.Label(frame_superior, text="Buscar:", font=fuente1, bg=color_principal, fg=color_secundario)
    ...
    boton_eliminar = tk.Button(frame_superior, text="Eliminar", font=fuente1, bg=color_secundario, fg=color_principal, width=10)
    boton_eliminar.config()
    boton_eliminar.pack(side="right", padx=(5, 0))

    boton_editar = tk.Button(frame_superior, text="Editar", font=fuente1, bg=color_secundario, fg=color_principal, width=10)
    boton_editar.config()
    boton_editar.pack(side="right", padx=5)

    boton_agregar = tk.Button(frame_superior, text="Añadir", font=fuente1, bg=color_secundario, fg=color_principal, width=10)
